...
static char *nm_strdup(nvm_t *virtual_machine, const char *p);
/* }}} */
...
static void prerun(nvm_t *vm)
{
  ...
  for (i = 3; i < vm->bytes_count; i++){
    ...
    if (vm->bytes[i] == FN_START){
      ...
      /* set its things */
      new_func->name = nm_strdup(vm, name);
      new_func->offset = i;
  ...
  switch (vm->bytes[vm->ip]){
    case NOP: {
      ...
      *new = integer;
      ...
      new_stack->var->name = nm_strdup(vm, string);
      ...
      *new = (*(INT *)SOS.ptr) / (*(INT *)FOS.ptr);
      ...
      /* set the frames name */
      new_frame->fn_name = nm_strdup(vm, string);
      new_frame->vars = NULL;
...
static char *nm_strdup(nvm_t *vm, const char *p)
{
  /* {{{ nm_strdup body */
